...
/* ───────────────────────────────────────────────── 8 · loader + hero reel ──
   ...
   The reel is 144 frames of the same shot. They are decoded off the main
   thread in small batches so the page stays interactive while they land, and
   the wordmark is not allowed to animate until it has itself decoded — the
   previous build ran its entrance while the logo was still arriving, which is
   why the mark appeared to fly in after it had already landed.               */

(function hero() {
  var load = $('#load'), bar = $('#loadBar'), pct = $('#loadPct');
  ...
  /* Six at a time keeps the network busy without starving the first paint. */
  var next = 0, LANES = 6;
  function pump() {
    while (next < srcs.length && LANES > 0) {
      ...
      (function (i) {
        ...
        im.onload = im.onerror = function () {
          ...
          ready++; LANES++;
          setP(ready / srcs.length);
          if (ready === 1) draw(0);
          if (ready === srcs.length) finish();
          else pump();
        };
        im.src = srcs[i];
      })(next++);
  ...
  function draw(i) {
    ...
    if (i === drawn) return;
    var im = imgs[i];
    /* Hold the last decoded frame rather than flashing a gap. */
    if (!im) { for (var k = i; k >= 0 && !imgs[k]; k--); im = imgs[Math.max(0, k)]; }
    if (!im) return;
    drawn = i;
  ...
  var job = Clock.add(function (y) {
    ...
    var p = clamp((y - top) / Math.max(1, run), 0, 1);
    prog.style.setProperty('--p', p);
    if (srcs.length) draw(Math.round(p * (srcs.length - 1)));
    B.classList.toggle('hero-out', p > .82);
  }, {
    resize: function () { if (sizer.size()) { drawn = -1; draw(Math.max(0, drawn)); } }
  });
  Clock.bind(hero, job, '30% 0px');

  /* The reel is the last thing in the document — deliberately, so the page is
     interactive before five megabytes of base64 are parsed. That means it is
     not on `window` yet when this runs, so the frames are handed over rather
     than read: whichever arrives second starts the load. */
  function boot(list) {
    srcs = list || [];
    imgs = new Array(srcs.length);
    if (!srcs.length) { setP(1); finish(); return; }
    setP(0); pump();
  }
  if (W.__SEQ) boot(W.__SEQ); else W.__seqReady = boot;

  /* A hard ceiling, so a slow connection never traps anyone on the loader. */
  setTimeout(function () { if (!B.classList.contains('is-live')) finish(); }, 12000);
})();
