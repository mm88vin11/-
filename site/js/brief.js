...
(function (w, d) {
  ...
  on(form, 'submit', function (ev) {
    ...
    // No backend is wired here: the payload is prepared and the UI confirms.
    // Point this at your endpoint when you deploy.
    var payload = {
      name: name.trim(), phone: tel,
      msg: (($('#brMsg') || {}).value || '').trim(),
      wants: state.wants, when: state.when, pain: state.pain,
      budget: (($('#brFrom') || {}).textContent || '') + '—' + (($('#brTo') || {}).textContent || '')
    };
    if (w.console && console.info) console.info('[brief] payload', payload);

    setTimeout(function () {
      steps.forEach(function (s) { s.classList.remove('is-on'); s.hidden = true; });
      var done = $('#brDone');
      if (done) { done.hidden = false; requestAnimationFrame(function () { done.classList.add('is-on'); }); }
