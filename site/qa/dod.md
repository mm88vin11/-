...
| ✅ | Every world pauses off screen | `liveWorlds` never exceeds 3 across all three scroll runs |
...
| ✅ | Three tiers, and a runtime downgrade | the cold-load run starts forced `high` and is measured stepping down; `?tier=low` run in §7 |
...
| ✅ | Contrast ≥ 4.5:1 (3:1 large) in every world | §7 — every text node measured against its resolved background |
...
| ⚠️ | 0 frames > 33 ms on desktop | Everything except `#hero` is at zero. `#hero` is a full-screen image blit per frame, which is free on a GPU and is CPU work under SwiftShader — see §6 and the per-section table |
